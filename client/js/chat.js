...
export function setupImagePreview() {
	...
	// 移动端长按图片处理
	// Mobile long press image handling
	setupImageLongPress(chatArea);
...
// 设置图片长按功能（移动端）
// Setup image long press functionality (mobile)
function setupImageLongPress(chatArea) {
	...
	chatArea.addEventListener('touchstart', function(e) {
		const target = e.target;
		if (target.tagName !== 'IMG' || !target.closest('.bubble-content')) return;
		
		touchStartTarget = target;
		
		// 长按 500ms 触发
		longPressTimer = setTimeout(() => {
			showImageActionMenu(target, e.touches[0].clientX, e.touches[0].clientY);
		}, 500);
	}, { passive: true });
	
	chatArea.addEventListener('touchmove', function() {
		// 移动时取消长按
		if (longPressTimer) {
...
function showImageActionMenu(imgElement, x, y) {
	// 移除已存在的菜单
	...
	const menu = createElement('div', { class: 'image-action-menu' });
	
	let menuItems = '';
	
	// 保存图片选项
	menuItems += `<div class="image-action-menu-item" data-action="save">💾 ${t('image.save', '保存图片')}</div>`;
	
	// 如果在微信中，显示识别二维码选项提示
	if (isWechat) {
		menuItems += `<div class="image-action-menu-item" data-action="qrcode">📱 ${t('image.scan_qrcode', '识别二维码')}</div>`;
	} else {
		// 非微信浏览器，提示保存后用微信扫描
		menuItems += `<div class="image-action-menu-item hint" data-action="hint">💡 ${t('image.save_and_scan', '保存后用微信扫一扫')}</div>`;
	}
	
	// 查看大图
	menuItems += `<div class="image-action-menu-item" data-action="preview">🔍 ${t('image.preview', '查看大图')}</div>`;
	...
	on(menu, 'click', function(ev) {
		...
		if (action === 'save') {
			// 保存图片
			saveImage(imgElement.src);
		} else if (action === 'qrcode') {
			// 微信中长按图片会自动识别二维码，这里提示用户
			showToastMsg(t('image.long_press_to_scan', '长按图片可识别二维码'), 'info');
		} else if (action === 'preview') {
			// 查看大图
			showImageModal(imgElement.src);
		} else if (action === 'hint') {
			// 提示信息
			showToastMsg(t('image.save_hint', '请先保存图片，然后打开微信扫一扫从相册选择'), 'info');
		}
...
function saveImage(src) {
	try {
		...
		if (src.startsWith('data:')) {
			link.click();
			showToastMsg(t('image.saving', '正在保存图片...'), 'info');
		} else {
			// 对于远程图片，尝试通过 fetch 下载
			fetch(src)
				.then(res => res.blob())
				.then(blob => {
					...
					showToastMsg(t('image.saved', '图片已保存'), 'success');
				})
				.catch(() => {
					// 降级处理：直接打开图片
					window.open(src, '_blank');
					showToastMsg(t('image.open_to_save', '请长按图片保存'), 'info');
				});
		}
	} catch (error) {
		console.error('Save image failed:', error);
		showToastMsg(t('image.save_failed', '保存失败，请长按图片保存'), 'error');
	}
}
