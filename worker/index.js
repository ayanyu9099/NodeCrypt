...
export class ChatRoom {  constructor(state, env) {
  ...
  async handleSession(connection, clientIP = 'unknown') {
    ...
    try {
      ...
      // alarm not supported in local dev
    }
          ...
          try {
            ...
            for (const member of members) {
              const client = this.clients[member];              if (this.isClientInChannel(client, channel)) {
                const memberList = members.filter((value) => {
                  return (value !== member ? true : false);
                }).map((id) => {
                  return { id: id, ip: this.clients[id] ? this.clients[id].ip : '' };
                });
  ...
  handleChannelMessage(clientId, decrypted) {
    ...
    try {
      ...
      const validMembers = Object.keys(decrypted.p).filter(member => {
        ...
        return isString(decrypted.p[member]) && targetClient && targetClient.connection && targetClient.channel === channel;
      });
      ...
      for (const member of validMembers) {
        ...
        if (targetClient.shared) {
          ...
          logEvent('channel-msg-skip', [clientId, member, 'target has no shared key'], 'debug');
        }
      }
  ...
  async cleanupOldConnections() {
    ...
    for (const clientId of clientsToRemove) {
      try {
        ...
        logEvent('cleanup-connection', [clientId, error], 'error');
      }
    }
